...
std::vector<HBox> lp::doc::linebreaking(const HBox& hbox) {
	...
	std::vector<HBox> ret;
	HBox current{{.width = 210_mm - 4_cm, .height = hbox.height}};
	...
	for (auto&& tmp : hbox.content) {
		...
		if (width >= current.width - 2_cm && std::holds_alternative<Glue>(tmp)) {
			...
			Dimension glueWidth = (current.width - width) / count;
