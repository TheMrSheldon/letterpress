...
namespace std {
	...
	public:
		...
		static void* operator new(std::size_t __frameSize, std::allocator_arg_t, _Alloc __alloc, _Args&...) {
			...
			::new (static_cast<void*>(std::addressof(__get_allocator(__frame, __frameSize))))
	...
	struct __generator_promise_base {
		...
		yield_value(std::ranges::elements_of<_Rng, _Allocator>&& __x) {
			return [](allocator_arg_t, _Allocator alloc,
					  auto&& __rng) -> generator<_Ref, std::remove_cvref_t<_Ref>, _Allocator> {
				for (auto&& e : __rng)
	...
	struct __generator_promise<generator<_Ref, _Value, _Alloc>, _ByteAllocator, _ExplicitAllocator> final
		...
		yield_value(std::ranges::elements_of<_Rng>&& __x) {
			static_assert(
					!_ExplicitAllocator, "This coroutine has an explicit allocator specified with std::allocator_arg "
										 "so an allocator needs to be passed "
	...
	public:
		using promise_type =
				__generator_promise<generator<_Ref, _Value>, __byte_allocator, true /*explicit Allocator*/>;
	};
